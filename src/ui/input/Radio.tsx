...
export function Radio({
  ...
  if (mstyle == "warning") {
    if (isCorrect === true) {
      validationBorder =
        "border-b-2 border-amber-500 shadow-[0_4px_12px_-4px_rgb(245 158 11)]";
    ...
    if (isCorrect === true) {
      validationBorder =
        "border-b-2 border-blue-500 shadow-[0_0_8px_0_theme(colors.blue.500)]";
    } else if (isCorrect === false) {
      validationBorder =
        "border-b-2 border-red-500 shadow-[0_4px_12px_-4px_rgba(239,68,68,0.5)]";
    }
  ...
  return (
    <label htmlFor={generatedId} className={wrapperClass}>
      ...
      {children && (
        <span className="font-medium leading-none contents">{children}</span>
      )}
    </label>
  );
}
